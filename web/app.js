import {forward,cloneWeights,backward,sgd,loss} from './engine.js';
...
const state={step:1,channel:0,inputChannel:0,x:13,y:13,index:0,neuron:0,classIndex:7,target:7,rate:.01,snapshot:'trained',sampleIndex:0,source:'sample',scanning:false,trainingSteps:0,trainingHistory:[]};
...
const app={state,
  lesson(){if (!Number.isInteger(currentPage.step)) {paintInput(); return;} const element=document.activeElement;const active=element?.id?'#'+element.id:['data-channel','data-neuron','data-index','data-slice','data-connection','data-softmax-digit'].filter(k=>element?.hasAttribute(k)).map(k=>`[${k}="${element.getAttribute(k)}"]`)[0];const opened=$$('#lesson details[open]').map(d=>d.querySelector('summary')?.textContent);renderLesson(app);$$('#lesson details').forEach(d=>{if(opened.includes(d.querySelector('summary')?.textContent))d.open=true;});if(active&&$(active))$(active).focus({preventScroll:true});paintInput();},
  ...
  resetTraining(){this.stopCalculation();state.reveal=9;state.trainingHistory=[];state.lastUpdate=null;},
  ...
  trainingStep(){const grads=backward(state.tensors,state.weights,state.target);if(!state.trainingHistory.length)state.trainingHistory.push(grads.loss);const index=grads.weights.conv1Weight.reduce((best,v,i)=>Math.abs(v)>Math.abs(grads.weights.conv1Weight[best])?i:best,0);state.lastUpdate={index,before:state.weights.conv1Weight[index],gradient:grads.weights.conv1Weight[index],rate:state.rate};state.weights=sgd(state.weights,grads.weights,state.rate);state.trainingSteps++;this.update();state.trainingHistory.push(loss(state.tensors.logits,state.target));updateModelControls();this.lesson();},
  editPixel(index,value){state.pixels[index]=value;state.source='edited';this.resetTraining();renderInputControls();this.update();},
  gradient(){return backward(state.tensors,state.weights,state.target);}
};
...
function renderContext() {
  ...
  $('#context-caption').textContent = isSample ? `MNIST test #${state.samples[state.sampleIndex].id}` : 'Your custom image';
  $('#context-sample').innerHTML = `${isSample ? '' : '<option value="-1">Custom</option>'}` + Array.from({length:10}, (_,digit) => `<option value="${digit}" ${isSample && digit === state.samples[state.sampleIndex].label ? 'selected' : ''}>Digit ${digit}</option>`).join('');
