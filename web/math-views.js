import { getPalette, signLegend } from './palette.js';
...
function accumulationSVG(trace, reveal, selected) {
  ...
  const x = i=>32+i*67;
  return `<svg class="accumulation-chart" viewBox="0 0 670 142" role="img" aria-label="Running sum of the nine products in row-major order"><path d="M24 ${y(0)}H650" stroke="var(--line)" stroke-dasharray="4 4"/><text x="13" y="${y(0)+4}" class="chart-zero">0</text><polyline points="${values.slice(0,reveal+1).map((value,i)=>`${x(i)},${y(value)}`).join(' ')}" fill="none" stroke="var(--accent)" stroke-width="2"/>${values.map((value,i)=>`<g opacity="${i<=reveal?1:.23}"><circle cx="${x(i)}" cy="${y(value)}" r="${i===selected+1?5:3}" fill="${i===selected+1?'var(--text)':'var(--accent)'}"/><text x="${x(i)}" y="${y(value)-13}" text-anchor="middle">${f(value,3)}</text><text x="${x(i)}" y="131" text-anchor="middle">${i?'+'+i:'start'}</text></g>`).join('')}</svg>`;
}
...
export function renderWorkedConvolution(app) {
  ...
  $('#lesson-work').innerHTML=`
    ...
    <p class="math-note">The final output includes every term, even when the replay above is paused partway through. ${layer===2?'The replay examines the selected input channel; its subtotal joins the other seven above.':''} Numeric labels are rounded. ${signLegend()}</p>`;
  ...
  function selectTerm(index) {
    ...
    if(px>=0&&py>=0&&px<n&&py<n){const context=$('#conv-source').getContext('2d');context.strokeStyle=getPalette().colors.change;context.lineWidth=2.4;context.strokeRect(px*8+1.2,py*8+1.2,5.6,5.6);}
  }
...
function fanInSVG(input, weights, output, bias, selected, target) {
  ...
  return `<svg class="fan-in-svg" viewBox="0 0 860 350" role="img" aria-label="Six actual connections into neuron ${target}; the output sum includes all ${input.length} connections"><text x="70" y="20">INPUT ACTIVATION</text><text x="270" y="20">WEIGHT</text><text x="433" y="20">PRODUCT</text><text x="680" y="20">ALL ${input.length} TERMS + BIAS</text>${rows.map((i,j)=>{const y=55+j*48,product=input[i]*weights[i],active=i===selected;return `<g data-connection="${i}" class="connection-line ${active?'chosen':''}" tabindex="0" role="button" aria-label="Inspect connection ${i}"><path d="M164 ${y}H402 M515 ${y}L638 178" fill="none" stroke="${product<0?'var(--negative)':'var(--positive)'}" stroke-width="${active?3:1+Math.abs(product)/scale*1.5}" opacity="${active?1:.55}"/><rect x="12" y="${y-16}" width="150" height="33" fill="${active?'var(--raised)':'var(--surface)'}" stroke="var(--line)"/><text x="21" y="${y+5}">x[${i}]  ${f(input[i],3)}</text><rect x="248" y="${y-12}" width="95" height="25" fill="var(--surface)"/><text x="259" y="${y+5}" fill="var(--negative)">× ${f(weights[i],4)}</text><rect x="402" y="${y-16}" width="110" height="33" fill="${product<0?'var(--negative-soft)':'var(--positive-soft)'}" stroke="${active?'var(--text)':'var(--line)'}"/><text x="417" y="${y+5}">${f(product,5)}</text></g>`;}).join('')}<circle cx="661" cy="178" r="28" fill="var(--bg)" stroke="var(--text)"/><text x="651" y="187" class="sigma-symbol">Σ</text><path d="M689 178H732" stroke="var(--text)"/><rect x="731" y="154" width="120" height="48" fill="var(--positive-soft)" stroke="var(--accent)"/><text x="745" y="184" class="fan-result">${f(output,5)}</text><text x="599" y="236">bias ${f(bias,5)}</text><text x="25" y="341">Shown: inputs ${start}–${start+5}. The remaining ${input.length-6} connections are included in the output.</text></svg>`;
}

function contributionBars(groups,bias) {
  const max=Math.max(...groups.map(g=>Math.abs(g.value)),Math.abs(bias),.001);
  return `<div class="contribution-bars"><div class="contribution-key"><span>← Decreases the score</span><span>Increases the score →</span></div>${[...groups,{label:'bias',value:bias}].map(group=>`<div class="contribution-bar-row"><span>${group.label}</span><div class="signed-track"><i style="left:${group.value<0?50-Math.abs(group.value)/max*50:50}%;width:${Math.abs(group.value)/max*50}%;background:${group.value<0?'var(--negative)':'var(--positive)'}"></i></div><strong>${f(group.value,5)}</strong></div>`).join('')}</div>`;
}
...
export function enhanceRelu(app) {
  ...
  plot.innerHTML=`<div><p class="eyebrow">THE FUNCTION, AT YOUR SELECTED VALUE</p><h4>ReLU(x) = max(0, x)</h4><p>${x<0?'The selected point lies on the flat segment. The output is zero.':x>0?'The selected point lies on the diagonal. The output equals the input.':'The selected point is at the corner. PyTorch uses derivative zero here.'}</p><p class="mono">Local derivative: <strong>${x>0?1:0}</strong></p></div><svg viewBox="0 0 430 290" role="img" aria-label="ReLU graph with selected input ${f(x,4)} and output ${f(y,4)}"><path d="M30 143H398 M200 270V15" stroke="var(--line)"/><path d="M45 143H200L355 35" fill="none" stroke="var(--accent)" stroke-width="3"/><path d="M${px(x)} 143V${py(y)}H200" fill="none" stroke="var(--negative)" stroke-dasharray="5 4"/><circle cx="${px(x)}" cy="${py(y)}" r="6" fill="var(--text)"/><text x="${Math.min(340,Math.max(35,px(x)+9))}" y="${Math.max(20,py(y)-15)}">(${f(x,3)}, ${f(y,3)})</text><text x="385" y="165">x</text><text x="210" y="22">ReLU(x)</text><text x="207" y="160">0</text><text x="35" y="278">Left: slope 0. Right: slope 1.</text></svg>`;
